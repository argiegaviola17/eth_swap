...
contract('EthSwap',([deployer,investor])=>{
    ...
    describe('JorgienaToken deployment',async()=>{
        it('contract has a name',async()=>{
            const name = await token.name();
            assert.equal(name,'Jorginena Token')
        })
    })
    ...
    describe('sellTokens()',async()=>{
        ...
        it('Allows user to instantly sell tokens from ethSwap for a fixed price',async()=>{
            ...
            await ethSwap.sellTokens(tokens('500'),{from:investor}).shoud.be.rejected;
        })
    })
})